...
import {
  Save, ChevronRight, RefreshCw, X, FlaskConical, Plus, History,
  FileEdit, Eye, AlignLeft, Trash2,
} from "lucide-react";
...
const BASE = "/whgc-seminars/manage-console";
...
function buildSeminarBlock(seminar: Seminar): string {
...
function ComposeHub() {
  ...
  async function createNew() {
    const brandConfig = BRAND_CONFIGS["whgc"];
    const res = await fetch("/api/newsletter/campaigns", {
      method: "POST",
    ...
    const data = await res.json();
    if (res.ok) {
      router.push(`${BASE}/newsletter-compose?id=${data.id}`);
    } else {
    ...
    <div className="p-6 space-y-8 max-w-4xl">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="admin-description mt-1">告知メルマガの新規作成・下書き編集・過去メール参照</p>
        </div>
      ...
      <section className="space-y-3">
        ...
        <div className="rounded-xl border border-border bg-card p-5 space-y-3">
          <p className="text-xs text-muted-foreground">WHGC ゲームチェンジャーズ・フォーラムのテンプレートで作成します。</p>
          <button
            onClick={createNew}
            className="flex flex-col items-start gap-1 rounded-lg border border-border bg-background px-4 py-3 text-left hover:border-primary hover:bg-primary/5 transition-colors w-full sm:w-auto"
          >
            ...
            <span className="text-xs text-muted-foreground leading-relaxed">
              ヘッダー：WHGC ゲームチェンジャーズ・フォーラム / 問い合わせ：info@whgcforum.org
            </span>
          </button>
          <p className="text-xs text-muted-foreground">
            件名・本文を入力し、下書き保存してから{" "}
            <Link href={`${BASE}/email-schedules`} className="text-primary underline hover:no-underline">
              配信スケジュール
            </Link>{" "}
            の告知集客メールに紐付けて配信します。
          </p>
                  ...
                  <tr key={c.id} className="hover:bg-muted/20">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2 justify-end">
                        <Link href={`${BASE}/newsletter-compose?id=${c.id}`}>
...
function NewsletterComposeInner() {
  ...
  if (!campaignId) return <ComposeHub />;
...
function ComposeEditor({ campaignId, router }: { campaignId: string; router: ReturnType<typeof useRouter> }) {
  ...
  useEffect(() => { loadSeminars(); }, [loadSeminars]);
  ...
  async function saveDraft() {
    ...
    try {
      await fetch(`/api/newsletter/campaigns/${campaignId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ subject, body, header_color: headerColor, footer_text: footerText || null }),
      });
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => router.push(`${BASE}/newsletter-compose`)}
      ...
      {showPreview && (
        <div className="fixed inset-0 z-50 flex flex-col bg-background">
          <div className="flex items-center justify-between border-b border-border bg-background px-6 py-3 shrink-0">
            ...
            <button onClick={() => setShowPreview(false)} className="rounded p-1.5 text-muted-foreground hover:bg-muted">
              <X className="size-4" />
            </button>
          </div>
