...
export default function WhgcSeminarsAdminMemberDomainsPage() {
  ...
  async function loadDomains() {
    try {
      const res = await fetch(`/api/member-domains?tenant=${TENANT}`);
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/member-domains", {
        ...
        body: JSON.stringify({ domain: d, tenant: TENANT }),
      });
      if (!res.ok) {
  ...
  async function handleDelete(domain: string) {
    ...
    try {
      const res = await fetch("/api/member-domains", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ domain, tenant: TENANT }),
      });
      if (!res.ok) {
        const err = await res.json();
